...
//========================================================================================
//! \file potential_functions.cpp
//! \brief implementation of modified bessel fnctions for calculating planettary potential 
/*
Adapted from the Cephes Math Library Release 2.8:  June, 2000
...
namespace PotentialFunctions {
	...
	Real k0(Real x){
		...
		if( x <= 0.0 ){
			std::cout << "k0 ERROR"; std::exit(EXIT_FAILURE);
			// mtherr( "k0", DOMAIN );
			return( HUGE_NUMBER );
		}
	...
	Real k0e(Real x ){
		...
		if( x <= 0.0 ){
			std::cout << "k0e ERROR"; std::exit(EXIT_FAILURE);
			return( HUGE_NUMBER );
		}
		...
		y = chbevl( 8.0/x - 2.0, B_K0, 25 ) / sqrt(x);
		return(y);
	}
	...
	Real k1(Real x){
		...
		if( z <= 0.0 )
			{
			std::cout << "k1 ERROR"; std::exit(EXIT_FAILURE);
			return( HUGE_NUMBER );
			}
	...
	Real k1e(Real x ){
		...
		if( x <= 0.0 )
			{
			std::cout << "k1e ERROR"; std::exit(EXIT_FAILURE);
			return( HUGE_NUMBER );
			}

		if( x <= 2.0 ){
			y = x * x - 2.0;
			y =  log( 0.5 * x ) * i1(x)  +  chbevl( y, A_K1, 11 ) / x;
			return( y * exp(x) );
		}

		return(  chbevl( 8.0/x - 2.0, B_K1, 25 ) / sqrt(x) );
	}
